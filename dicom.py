...
class image_lbl(QLabel):
    def __init__(self):
        ...
        self.setText('\n\n Drop Image Here \n\n')
        self.setFont(QFont('caveat brush',15))
        self.setStyleSheet('''
            QLabel{
...
class Image_Drag_Drop(QWidget):
    ...
    def clear_image(self):
            ...
            self.photoViewer.setFont(QFont('caveat brush',15))
...
class Ui_DICOM(object):
    def setupUi(self, DICOM):
        ...
        self.main.setStyleSheet("background-color:black;")
...
"}")
        ...
        self.nav_drw.setStyleSheet("background-color: black;")
        ...
        self.nav_mnu.setStyleSheet("QFrame{background-color: black;}\n"
"\n"
"QPushButton{\n"
"padding: 10px 10px;\n"
...
"}")
        ...
        self.pages_frm.setStyleSheet("QFrame{background-color: black;}\n"
"\n"
"")
        ...
        self.main_pages_vwr.setStyleSheet("background-color: black;")
...
"border-radius:10px;")
        ...
        self.dicom_content.setStyleSheet("background-color:black;")
...
"}")
        ...
        self.seg_btn = QtWidgets.QPushButton(self.control_wid)
        self.seg_btn.setGeometry(QtCore.QRect(230, 40, 121, 71))
        font = QtGui.QFont()
        font.setPointSize(11)
        font.setBold(True)
        font.setWeight(75)
        self.seg_btn.setFont(font)
        self.seg_btn.setText("Segmentation")
        self.seg_btn.setStyleSheet("QPushButton{\n"
"color: rgb(255, 255, 255);\n"
...
"}")
        ...
        self.Filters_pg.setStyleSheet("background-color: black;")
...
"}")
        ...
        self.filters_drw.setStyleSheet("background-color: black;")
        ...
        self.filters_mnu.setStyleSheet("QFrame{background-color: black;}\n"
"\n"
"QPushButton{\n"
"padding: 10px 10px;\n"
...
"}")
        ...
        self.About_pg.setStyleSheet("background-color:black;")
        ...
        self.label.setText("")
        self.label.setPixmap(QtGui.QPixmap("images/hh.jpg"))
    ...
    def retranslateUi(self, DICOM):
        ...
        self.label_5.setText(_translate("DICOM", "Hesham Atef"))
        self.running=False
...
##########################################################################################
        ...
        self.seg_btn.clicked.connect(self.bones_segmentation)
...
#Dicom functions section
    ...
    def view_dicom_slices(self,val):
        if len(planes_list)<1:
            ...
            if self.axial_tgl.isChecked() and not self.coronal_tgl.isChecked() and not self.sagital_tgl.isChecked():
                ...
                #Checking if segmentation button checked or not
                if self.seg_btn.isChecked():
                    img=self.tresh_and_overlay(plane)
                else:
            ...
            #For sliding through axial-coronal slices only
                ...
                if self.seg_btn.isChecked():
                    img=self.tresh_and_overlay(plane)
                    img1=self.tresh_and_overlay(plane1)
                else:
                    resized_plane=cv2.resize(plane, dsize=(337,200), interpolation=cv2.INTER_CUBIC)
                    img=imgia.array2qimage(resized_plane/resized_plane.max()*255)
            ...
            #For sliding through axial-sagittal slices only
                ...
                if self.seg_btn.isChecked():
                    img=self.tresh_and_overlay(plane)
                    img2=self.tresh_and_overlay(plane2)
                else:
                    resized_plane=cv2.resize(plane, dsize=(337,200), interpolation=cv2.INTER_CUBIC)
                    img=imgia.array2qimage(resized_plane/resized_plane.max()*255)
        ...
        if self.seg_btn.isChecked():
            ...
            dialog.setStyleSheet('''
                                *{ color: black; font-weight: bold;}
                                ...
                                QLabel{ color: rgb(0,0,0);font-weight: bold; font-size:12px;}
    ...
    def tresh_and_overlay(self,plane):
        ...
        try:
            ...
            #normalization
            resized_plane=resized_plane/resized_plane.max()*255
            
            #Thresholding
            thresh=cv2.threshold(resized_plane,seg_val,255,cv2.THRESH_BINARY)[1]
            
            #Converting images to RGB
            thresh_3d = cv2.cvtColor(np.uint8(thresh),cv2.COLOR_GRAY2RGB)
            plane_3d= cv2.cvtColor(np.uint8(resized_plane),cv2.COLOR_GRAY2RGB)

            #Getting the red channel only
            thresh_3d[:,:,1] = 0
            thresh_3d[:,:,2] = 0

            
            #overlaying the mask over the image
            seg_img = cv2.addWeighted(thresh_3d,1, plane_3d,1,0)

            seg_img=imgia.array2qimage(seg_img)
